...
volumecontrol::volumecontrol(int vol_pin){
    pinMode(vol_pin, OUTPUT);
    _pin = vol_pin;
}
void volumecontrol::offOn(float otherBPM){
    _bpm = otherBPM;
...
frequencyScale::frequencyScale(int freq_pin){
...
void frequencyScale::changePitch(int note){
    ...
    analogWrite(_pin, minPitch);

    for (int y = 0; y <= maxPitch; y++){
       if (y <= 255){
         analogWrite(_pin, 2);
       } 
       else{
